...
def moderation_dashboard(request):
    ...
    flagged_reviews = Review.objects.filter(
        flags__is_resolved=False
    ).distinct().select_related('user', 'company')
    
    # Statistics
    stats = {
        'pending': Review.objects.filter(is_approved=False, approval_requested=True).count(),
...
def flag_review(request, review_id):
    ...
    if request.method == 'POST':
        ...
        if existing_flag:
            return JsonResponse({
                'success': False,
                'error': 'Siz bu sharhni allaqachon belgiladingiz'
            })
        ...
        return JsonResponse({
            'success': True,
            'message': 'Sharh moderatsiyaga yuborildi'
        })
...
def request_verification(request, company_id):
    ...
    if request.method == 'POST':
        if company.is_verified:
            return JsonResponse({
                'success': False,
                'error': 'Biznesingiz allaqachon tasdiqlangan'
            })
        ...
        if document:
            ...
            return JsonResponse({
                'success': False,
                'error': 'Tasdiqlash hujjatini yuklang'
            })
...
def export_reviews_pdf(request, company_id):
    ...
    try:
        ...
        elements.append(Paragraph(f"<b>Total Reviews:</b> {reviews.count()}", info_style))
        elements.append(Paragraph(f"<b>Average Rating:</b> {company.average_rating}/5.0", info_style))
        ...
        # Return response
        buffer.seek(0)
        response = FileResponse(buffer, as_attachment=True, filename=f'{company.name}_reviews.pdf')
...
def export_reviews_excel(request, company_id):
    ...
    try:
        ...
        # Headers
        headers = ['Date', 'User', 'Rating', 'Title', 'Review', 'Helpful Votes', 'Response']
        ws.append(headers)
        
        # Style headers
        header_fill = PatternFill(start_color="00D68B", end_color="00D68B", fill_type="solid")
        ...
        response['Content-Disposition'] = f'attachment; filename={company.name}_reviews.xlsx'
