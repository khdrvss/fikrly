...
def home(request):
    ...
    if request.user.is_authenticated and not request.session.get("_redirected_once"):
        request.session["_next_after_login"] = request.session.get(
...
def manager_request_approval(request, pk: int):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request, "Tasdiqlash so‘rovi yuborildi. Adminlar ko‘rib chiqishadi."
            )
            return redirect("business_dashboard")
    else:
...
def claim_company(request, pk: int):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                send_telegram_message(
                    admin_msg, chat_ids=settings.TELEGRAM_ADMIN_CHAT_IDS
                )
            except Exception:
                pass
            ...
            verify_url = request.build_absolute_uri(
                reverse("verify_claim", args=[token])
            )
            ...
            messages.success(
                request, "Tasdiqlash havolasi email manzilingizga yuborildi."
            )
            return redirect("company_detail", pk=company.pk)
    else:
    ...
    return render(
        request, "pages/company_claim.html", {"company": company, "form": form}
    )


def verify_claim(request, token: str):
    try:
        claim = CompanyClaim.objects.select_related("company", "claimant").get(
            token=token
        )
...
def report_review(request, pk: int):
    ...
    if request.method == "POST":
        ...
        if entry["count"] >= max_reports:
            messages.error(
                request, "Juda ko‘p urinish. Bir daqiqadan so‘ng qayta urinib ko‘ring."
            )
            return redirect("company_detail", pk=review.company.pk)
        ...
        if form.is_valid():
            ...
            reason = dict(report._meta.get_field("reason").choices).get(
...
def company_detail(request, pk: int):
    ...
    company.assessment = compute_assessment(
        float(company.rating), int(company.review_count)
    )
    ...
    liked_state = False
    if request.user.is_authenticated:
        liked_state = CompanyLike.objects.filter(
