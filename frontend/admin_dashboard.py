...
from frontend.models import (
    Company, Review, UserProfile, ActivityLog,
    ReviewReport, CompanyClaim, DataExport
)
...
def admin_dashboard(request):
    ...
    recent_activity = ActivityLog.objects.select_related('actor', 'company', 'review').order_by('-created_at')[:20]
    ...
    for i in range(6, -1, -1):
        ...
        day_stats = {
            ...
            'reviews': Review.objects.filter(created_at__range=(day_start, day_end)).count(),
            'companies': Company.objects.filter(id__isnull=False).count(),  # Placeholder
        }
