...
def analytics_dashboard(request, company_id):
    ...
    # Rating distribution
    rating_distribution = list(reviews.values('rating').annotate(
        count=Count('id')
    ).order_by('rating'))
    ...
    pending_responses = Review.objects.filter(
        company=company,
        is_approved=True,
        owner_response_text__isnull=True
...
def calculate_response_rate(company):
    ...
    responded = Review.objects.filter(
        company=company,
        is_approved=True
    ).exclude(owner_response_text='').count()
...
def setup_2fa(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'enable':
            ...
            return render(request, 'frontend/2fa_setup.html', {
                'qr_code': qr_code,
                'secret': secret,
                'step': 'scan'
            })
