...
class SearchTests(TestCase):
    def setUp(self):
        self.category = BusinessCategory.objects.create(
            name="Test Category", slug="test-category", icon_svg="<path d='...'/>"
        )
    ...
    def test_search_suggestions_api(self):
        ...
        self.assertTrue(
            any(
                r["type"] == "category" and r["name"] == "Test Category"
                for r in results
            )
        )
        self.assertTrue(
    ...
    def test_search_suggestions_empty(self):
        response = self.client.get(
            reverse("search_suggestions_api"), {"q": "NonExistent"}
        )
        self.assertEqual(response.status_code, 200)
...
class ReviewFileUploadTests(TestCase):
    ...
    def test_small_file_upload_validation(self):
        ...
        small_file = SimpleUploadedFile(
            "small_receipt.jpg", small_content, content_type="image/jpeg"
        )
