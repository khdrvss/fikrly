...
class FormValidator {
  constructor(form = null) {
    this.form = form || document.querySelector('form[data-validate]') || document.querySelector('form');
    if (this.form) {
      this.init();
    }
  }
...
class SearchAutocomplete {
  constructor(input, options = {}) {
    const langCode = (document.documentElement.lang || 'uz').toLowerCase();
    ...
    this.options = {
      ...
      searchUrl: options.searchUrl || '/api/search-suggestions/',
      companiesOnly: options.companiesOnly !== undefined
        ? options.companiesOnly
        : (input.dataset.companiesOnly !== '0'),
      recentSearchesKey: options.recentSearchesKey || 'fikrly_recent_searches',
      maxRecentSearches: options.maxRecentSearches || 5,
      placeholder: options.placeholder || (isRu ? 'Поиск...' : 'Qidiruv...'),
  ...
  async fetchResults(query) {
    ...
    if (this.options.companiesOnly) {
      url.searchParams.set('companies_only', '1');
    }
    ...
    return this.options.companiesOnly ? items.filter(item => item.type === 'company') : items;
  }
  ...
  showRecentSearches() {
    ...
    this.renderDropdown(isRu ? 'Недавние запросы' : 'Oxirgi qidiruvlar');
  }
    ...
    if (results.length === 0) {
      this.dropdown.innerHTML = `
        <div class="p-4 text-center text-gray-500">
          ...
          <p class="text-sm">${(document.documentElement.lang || '').toLowerCase().startsWith('ru') ? 'Ничего не найдено' : 'Hech narsa topilmadi'}</p>
        </div>
      `;
      this.open();
      return;
        ...
        ${this.results.map((result, index) => `
          <button type="button" 
            ...
            ${result.type === 'recent'
              ...
              : result.logo
                ? `<img src="${result.logo}" alt="${result.name}" class="w-8 h-8 rounded-lg object-cover flex-shrink-0" onerror="this.style.display='none'">`
  ...
  selectResult(result) {
    if (result.type === 'recent') {
      ...
      if (result.url && result.type === 'company') {
...
class ScrollToTop {
  ...
  static init(options = {}) {
    ...
    // Avoid duplicate if base.html or another script already created one
    if (document.getElementById('scroll-to-top')) {
      ScrollToTop.instance = { button: document.getElementById('scroll-to-top'), toggleButton: () => {} };
    ...
    // Use inline style for safe-area-aware bottom — Tailwind cannot compute env()
    const side = defaults.position === 'bottom-left' ? 'left: 16px;' : 'right: 16px;';
    ...
    // Show/hide on scroll — opacity only, no scale
    let isVisible = false;
    const toggleButton = () => {
  ...
  document.querySelectorAll('form').forEach(form => {
    form.addEventListener('submit', function(e) {
      ...
      if (submitBtn && !submitBtn.disabled) {
        ...
        if (this.id === 'reviewForm' && window.Toast) {
          Toast.success('Sharhingiz yuborilmoqda...');
        }
...
// ============================================
// Legacy duplicate initializer removed in favor of the primary
// DOMContentLoaded bootstrap above.
