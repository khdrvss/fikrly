...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        expired_exports = DataExport.objects.filter(
            completed_at__lt=cutoff_date, status="completed"
        )
        ...
        if count == 0:
            self.stdout.write(
                self.style.SUCCESS(f"No expired exports found (older than {days} days)")
            )
            return

        if dry_run:
            self.stdout.write(
                self.style.WARNING(f"DRY RUN: Would delete {count} exports:")
            )
            for export in expired_exports:
                self.stdout.write(
                    f"  - Export #{export.id} ({export.format}) from {export.completed_at}"
                )
        else:
            ...
            for export in expired_exports:
                if export.file:
                    try:
                        ...
                        logger.error(
                            f"Failed to delete file for export #{export.id}: {e}"
                        )
