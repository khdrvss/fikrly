...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "phone",
            type=str,
            help="Recipient phone in international digits (e.g., 998901234567)",
        )
    ...
    def handle(self, *args, **options):
        ...
        if not (phone_digits.startswith("998") and len(phone_digits) == 12):
            raise CommandError(
                "Phone must be 12 digits starting with 998, e.g., 998901234567"
            )
        code = "123456"
        ...
        if ok:
            self.stdout.write(
                self.style.SUCCESS(
                    f"Test SMS sent (or printed in DEBUG) to {phone_digits}. Code: {code}"
                )
            )
        else:
